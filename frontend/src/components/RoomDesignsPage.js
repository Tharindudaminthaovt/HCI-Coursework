...
const RoomDesignsPage = () => {
      ...
      try {
        ...
        if (response.data.designs.length > 0) {
          ...
          const detailResponse = await axios.get(
            `http://localhost:3001/api/admin/room-designs/${response.data.designs[0]._id}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
  ...
  const createRoom = (scene, design) => {
    ...
    if (shape === "rectangular" || shape === "custom") {
      ...
      // Right wall for secondary section is integrated with the main section's left wall

      // No front walls to create open view effect
    }
    ...
    if (design.furniture && design.furniture.length > 0) {
      design.furniture.forEach((item) => {
        // Create simple cube for furniture
                ...
                <div className="bg-white p-6 rounded-lg shadow-md">
                  ...
                  <div className="border rounded-md p-1 bg-gray-50">
                    {activeTab === "2d" ? (
                      ...
                      <div ref={threeDContainerRef} className="h-96 w-full">
